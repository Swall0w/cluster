...
class KMeans(object):
    ...
    def fit_predict(self,X):
        ...
        for _ in range(self.max_itr):
            new_pred = np.array([np.array([self._euc_distance(p, centroid)for centroid in self.centers]).argmin() for p in X ])
            if np.all(new_pred == pred):
                break
                pred = new_pred
            self.centers = np.array([X[pred == i].mean(axis=0) for i in range(self.num_clusters)])
        return pred
